...
class TimesFMFinetuner:
    ...
    def _train_epoch(self, train_loader: DataLoader,
        ...
        if self.config.distributed:
            ...
            avg_loss = (avg_loss_tensor / torch.distributed.get_world_size()).item()
...
def prepare_datasets(series_list: List[np.ndarray],
                     context_length: int,
                     horizon_length: int,
                     freq_type: int = 0,
...
def plot_predictions(
    ...
    """
    Plot model predictions against ground truth for a single sample from the validation data.
    This function will now plot one random sample from the combined validation dataset.
    """
    ...
    # Get a random sample from the combined validation dataset
    ...
    plt.ylabel("Value")
    plt.title(f"TimesFM Predictions vs Ground Truth (Sample {idx})")
    plt.legend()
    plt.grid(True)

    if save_path:
        plt.savefig(save_path.replace(".png", f"_sample_{idx}.png"))  # Save with sample index
        print(f"Plot saved to {save_path.replace('.png', f'_sample_{idx}.png')}")
...
def get_data(context_len: int,
    ...
    tickers = ["AAPL", "MSFT"]  # Add more tickers here
    all_series = []
    for ticker in tickers:
        df = yf.download(ticker, start="2010-01-01", end="2019-01-01")
        if not df.empty:
            all_series.append(df["Close"].values)
        else:
            print(f"Warning: No data downloaded for {ticker}")

    if not all_series:
        raise ValueError("No time series data available for any ticker.")
    ...
    print(f"Created datasets for {len(tickers)} stocks:")
...
def single_gpu_example():
    ...
    # Plot predictions for a few samples from the validation set
    for _ in range(3):  # Plot 3 random samples
        plot_predictions(
            model=model,
            val_dataset=val_dataset,
            save_path="timesfm_predictions.png",  # The save_path will be modified to include sample index
        )
